...
function handleFormSubmit(event) {
    ...
    axios.post("http://localhost:4334/expense/add-expense", obj, {
        headers: {
...
window.onload = async () => {
    ...
    try {
        const response = await axios.get("http://localhost:4334/expense/get-expense", {
            headers: headers
        })
...
async function DeleteUser(id) {
    ...
    try {
        const res = await axios.delete(`http://localhost:4334/expense/delete-expense/${id}`, {
            headers: {
                'Content-Type': 'application/json',
                "Authorization": `Bearer ${token}`,
            }
...
document.getElementById('rzp-button').onclick = async function (e) {
    ...
    const response = await axios.get('http://localhost:4334/premiummember', {
        headers: {
            'Content-Type': 'application/json',
            "Authorization": `Bearer ${token}`,
        }
    ...
    var options =
    {
        ...
        "handler": async function (response) {
            const res = await axios.post('http://localhost:4334/updatetransactionstatus', {
...
async function showLeaderboard() {
    ...
    const token = localStorage.getItem('token');
    const userleaderboardarray = await axios.get('http://localhost:4334/premium/showleaderboard', {
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        }
...
document.getElementById('download-expense').addEventListener('click' , async()=>{
    ...
    try{
        const result = await axios.get('http://localhost:4334/download', {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
            }
...
async function showDownloadUrls(){
    try{
        const token=localStorage.getItem('token')
        const getUrls = await axios.get('http://localhost:4334/get-all-urls', {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
            }
